...
def train(data_path: str, output_dir: str, seeds: list[int] | None = None) -> None:
    ...
    logger.info(f"  [2] Hidden Dropout       : 0.2             (original: 0.1)")
    logger.info(f"  [4] Attention Dropout    : 0.2             (original: 0.1)")
    ...
    logger.info(f"  [7] Loss ponderada       : pesos {CLASS_WEIGHTS.tolist()}  (Camada 1)")
    logger.info(f"  [8] Threshold tuning     : ativado pós-treino na validação  (Camada 1)")
    logger.info(f"  [9] Multi-seed averaging : seeds={seeds}  (Camada 1)")
